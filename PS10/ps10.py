...
class Hand(object):
    ...
    def update(self, word):
        ...
        for letter in word:
            self.handDict[letter] = self.handDict[letter] - 1
    ...
    def containsLetters(self, letters):
        ...
        tempHand = deepcopy(self.handDict)
    ...
    def __eq__(self, other):
        ...
        for letter in self.handDict.keys():
            if other.get(letter, 0) or self.handDict[letter] != other[letter]:
                return False
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        for key in self.hand.handDict.keys():
           for i in range(self.hand.handDict[key]):
                letters += key

        for j in range(1, len(letters) + 1):
            combinations_ = [i for i in itertools.permutations(letters, j)]
            
            for tuple in combinations_:
        ...
        for tuple in all_combinations:
            ...
            if wordlist.containsWord(test_word):
                test_score = getWordScore(test_word)
                #print test_score, test_word
